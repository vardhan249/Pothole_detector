...
namespace Pothole {
  void
  Pothole_detector::update_fps(gint id) {
  ...
  int
  Pothole_detector::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Pothole_detector::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Pothole_detector::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  Pothole_detector::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
  ...
  gboolean
  Pothole_detector::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Pothole_detector::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Pothole_detector::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Pothole_detector::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Pothole_detector::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Pothole_detector::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Pothole_detector::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        Pothole::Pothole_detector::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", Pothole::Pothole_detector::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!Pothole::Pothole_detector::set_tracker_properties(nvtracker)) {
  ...
  void Pothole_detector::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/pothole_Yolov5/config_infer_primary_yolov5s.txt");
...
int main(int argc, char *argv[]) {
  ...
  Pothole::Pothole_detector Pothole_detector;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("Pothole_detector DeepStream App");
  group = g_option_group_new("space-empty", "space-occupied", NULL, NULL, NULL);
  g_option_group_add_entries(group, Pothole_detector.entries);
  ...
  gint sources = Pothole_detector.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  Pothole_detector.setPaths(num_sources);
  ...
  if (Pothole_detector.display_off) {
  ...
  int fail_safe = Pothole_detector.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, Pothole_detector.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      Pothole_detector.tiler_src_pad_buffer_probe, NULL, NULL);
  }
